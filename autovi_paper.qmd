...
The `autovi` R package can be extended to accommodate other classes of models beyond linear regression, such as generalized linear models (`glm`). This is achieved by substituting the relevant overridable modules, as illustrated in @fig-autovi-diag.
...
```{r echo = TRUE}
AUTO_GLM_VI <- new_class(AUTO_VI, class_name = "AUTO_GLM_VI")
register_method(
  AUTO_GLM_VI,
...
auto_glm_vi <- AUTO_GLM_VI$instantiate
```
...
if (!file.exists("cached_data/autovi_glm_checker_demo_print.rds")) {
  glm_checker <- auto_glm_vi(glm(y ~ x, family = "poisson"),
                           keras_model = get_keras_model("vss_phn_32"))
  glm_checker$check()
  saveRDS(glm_checker$..str..(), "cached_data/autovi_glm_checker_demo_print.rds")
} else {
