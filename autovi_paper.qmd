...
This distance measure depends on knowledge of the true residual distribution, which is often unknown. However, it can be estimated by identifying or approximating a mapping from a set of residuals to its corresponding distance measure. The computer vision model presented in @li2024automated approximates this mapping. It is trained on a large number of synthetic regression models, where the data-generating process is known, allowing the distance measure to be explicitly calculated. The model takes a residual plot as input and outputs the corresponding distance measure. Additional details are provided in @li2024automated.
