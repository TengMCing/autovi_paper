...
Equation \ref{eq:kl-1} can be adapted to different forms to account for various model violations. For instance, in the case of a linear regression model where necessary higher-order predictors, $\boldsymbol{Z}$, and their corresponding parameter, $\boldsymbol{\beta}_Z$, are omitted, the distance measure can be expanded as follows:
