...
%DIF DEL autovi_paper_submission1.tex   Wed May 14 13:06:49 2025
%DIF ADD autovi_paper.tex               Wed May 14 13:03:54 2025
%DIF 2a2
...
\author{
Weihao Li\DIFdelbegin %DIFDELCMD < \addressnum{1}%%%
\DIFdelend \DIFaddbegin \addressnum{1, 2}\DIFaddend ,
...
%DIF 249c259
%DIF < \hspace*{1ex} Email: \texttt{weihao.li@monash.edu}
%DIF -------
\hspace*{1ex} Email: \texttt{patrick.li@anu.edu.au}     %DIF >
%DIF -------
}
...
\begin{abstract}
\DIFdelbegin \DIFdel{Visually assessing }\DIFdelend \DIFaddbegin \DIFadd{Visual assessment of }\DIFaddend residual plots is a common \DIFdelbegin \DIFdel{advice for linear model
diagnostics, however this approach requires manual human evaluationand
...
analysts. Using a }\DIFdelend \DIFaddbegin \DIFadd{approach for diagnosing
linear models, but it relies on manual evaluation, which does not scale
well and can lead to inconsistent decisions across analysts. The }\DIFaddend lineup
protocol, \DIFdelbegin \DIFdel{where the residual plot is embedded
}\DIFdelend \DIFaddbegin \DIFadd{which embeds the observed plot }\DIFaddend among null plots, can \DIFdelbegin \DIFdel{help to alleviate inconsistency, }\DIFdelend \DIFaddbegin \DIFadd{reduce
subjectivity }\DIFaddend but requires even more human effort. \DIFdelbegin \DIFdel{This is the type of task that in }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend today's \DIFdelbegin \DIFdel{world we
might employ a robot to do the tedious work for a human. Here we
describe }\DIFdelend \DIFaddbegin \DIFadd{data-driven
world, such tasks are well-suited for automation. We present }\DIFaddend a new R
package that \DIFdelbegin \DIFdel{includes }\DIFdelend \DIFaddbegin \DIFadd{uses }\DIFaddend a computer vision model \DIFdelbegin \DIFdel{for
automated assessment }\DIFdelend \DIFaddbegin \DIFadd{to automate the evaluation }\DIFaddend of
residual plots\DIFdelbegin \DIFdel{, and an }\DIFdelend \DIFaddbegin \DIFadd{. An }\DIFaddend accompanying Shiny app \DIFaddbegin \DIFadd{is provided }\DIFaddend for ease of use.
\DIFdelbegin \DIFdel{For a user-provided }\DIFdelend \DIFaddbegin \DIFadd{Given a }\DIFaddend sample of residuals, \DIFdelbegin \DIFdel{it predicts a measure of }\DIFdelend \DIFaddbegin \DIFadd{the model predicts a }\DIFaddend visual signal strength
(VSS) and \DIFdelbegin \DIFdel{provides a suite of
}\DIFdelend \DIFaddbegin \DIFadd{offers }\DIFaddend supporting information to \DIFdelbegin \DIFdel{assist the analyst decide on the
appropriateness their }\DIFdelend \DIFaddbegin \DIFadd{help analysts assess }\DIFaddend model
fit.
\end{abstract}
...
\texttt{lmtest} \citep{lmtest} provides a suite of conventional tests\DIFdelbegin \DIFdel{.
The }\DIFdelend \DIFaddbegin \DIFadd{,
while the }\DIFaddend \texttt{stats} package \citep{stats} offers standard
diagnostic plots such as residuals vs.~fitted values, quantile-quantile
(Q-Q) plots, and residuals vs.~leverage plots. \DIFdelbegin \DIFdel{Packages }\DIFdelend \DIFaddbegin \DIFadd{Additional packages }\DIFaddend like
\texttt{jtools} \citep{jtools}, \texttt{olsrr} \citep{olsrr},
\texttt{rockchalk} \citep{rockchalk}, and \texttt{ggResidpanel}
\citep{ggresidpanel} \DIFdelbegin \DIFdel{provide }\DIFdelend \DIFaddbegin \DIFadd{deliver }\DIFaddend similar graphical diagnostics, often with
\DIFdelbegin \DIFdel{alternative }\DIFdelend \DIFaddbegin \DIFadd{enhanced }\DIFaddend aesthetics or interactive features. \DIFdelbegin \DIFdel{All of these tools deliver the types of
}\DIFdelend \DIFaddbegin \DIFadd{These tools collectively
produce the core }\DIFaddend diagnostic plots outlined in the classical text by
\citet{cook1982residuals}. The \texttt{ecostats} package
\citep{warton_global_2023} \DIFdelbegin \DIFdel{incorporates }\DIFdelend \DIFaddbegin \DIFadd{extends these diagnostics by incorporating
}\DIFaddend simulation envelopes into residual plots\DIFdelbegin \DIFdel{, while }\DIFdelend \DIFaddbegin \DIFadd{. Meanwhile, }\DIFaddend \texttt{DHARMa}
\citep{dharma} compares empirical quantiles (0.25, 0.5, and 0.75) of
scaled residuals to their theoretical counterparts\DIFdelbegin \DIFdel{. }\texttt{\DIFdel{DHARMa}} %DIFAUXCMD
\DIFdel{is particularly focused on detecting }\DIFdelend \DIFaddbegin \DIFadd{, with a strong focus
on identifying }\DIFaddend model violations such as heteroscedasticity, \DIFdelbegin \DIFdel{incorrect }\DIFdelend \DIFaddbegin \DIFadd{misspecified
}\DIFaddend functional forms, and issues specific to generalized linear and
mixed-effect models, like over/under-dispersion. It also \DIFdelbegin \DIFdel{includes }\DIFdelend \DIFaddbegin \DIFadd{provides
}\DIFaddend conventional test annotations to \DIFdelbegin \DIFdel{help avoid }\DIFdelend \DIFaddbegin \DIFadd{reduce the risk of }\DIFaddend misinterpretation.
...
violations. \citet{li2024plot} demonstrated that visual \DIFdelbegin \DIFdel{methods }\DIFdelend \DIFaddbegin \DIFadd{inference
methods, particularly those }\DIFaddend using the lineup protocol
\citep{buja2009statistical}\DIFdelbegin \DIFdel{for assessing residuals
are more useful, and also perform more practically }\DIFdelend \DIFaddbegin \DIFadd{, offer more practical and reliable
assessments of residual patterns }\DIFaddend than conventional tests\DIFdelbegin \DIFdel{due to their reduced sensitivity to }\DIFdelend \DIFaddbegin \DIFadd{, as they are
less sensitive to }\DIFaddend minor departures. Packages such as \texttt{nullabor}
\citep{nullabor}, \texttt{HLMdiag} \citep{loy2014hlmdiag}, and
\texttt{regressinator} \citep{regressinator} \DIFdelbegin \DIFdel{, enable }\DIFdelend \DIFaddbegin \DIFadd{support this approach by
enabling }\DIFaddend users to compare observed residual plots with plots \DIFdelbegin \DIFdel{of samples from null distributions, }\DIFdelend \DIFaddbegin \DIFadd{generated
under null hypothesis, thereby }\DIFaddend helping to quantify the significance of
any detected patterns.

\DIFdelbegin \DIFdel{However, as discussed }\DIFdelend \DIFaddbegin \DIFadd{As noted }\DIFaddend in \citet{li2024automated}, the lineup protocol has significant
limitations in large-scale applications due to \DIFdelbegin \DIFdel{dependence }\DIFdelend \DIFaddbegin \DIFadd{its reliance }\DIFaddend on human
labor. \DIFdelbegin \DIFdel{Thus}\DIFdelend \DIFaddbegin \DIFadd{To overcome this constraint}\DIFaddend , a computer vision model was
developed \DIFdelbegin \DIFdel{with an associated }\DIFdelend \DIFaddbegin \DIFadd{alongside a corresponding }\DIFaddend statistical testing procedure to
\DIFdelbegin \DIFdel{automate }\DIFdelend \DIFaddbegin \DIFadd{autmoate }\DIFaddend the assessment of residual plots. \DIFdelbegin \DIFdel{This model takes }\DIFdelend \DIFaddbegin \DIFadd{The model takes as input }\DIFaddend a
residual plot and a \DIFdelbegin \DIFdel{vector }\DIFdelend \DIFaddbegin \DIFadd{set }\DIFaddend of auxiliary variables (such as the number of
observations) \DIFdelbegin \DIFdel{as
inputs and outputs the }\DIFdelend \DIFaddbegin \DIFadd{and outputs a }\DIFaddend predicted visual signal strength (VSS). This
\DIFdelbegin \DIFdel{strength estimates the distance }\DIFdelend \DIFaddbegin \DIFadd{VSS estimates the degree of deviation }\DIFaddend between the residual distribution
of the fitted \DIFdelbegin \DIFdel{regression }\DIFdelend model and the reference distribution \DIFdelbegin \DIFdel{assumed }\DIFdelend \DIFaddbegin \DIFadd{expected }\DIFaddend under
...
a }\DIFdelend \DIFaddbegin \DIFadd{along with a companion }\DIFaddend web interface, \texttt{autovi.web}\DIFdelbegin \DIFdel{to make it easy for }\DIFdelend \DIFaddbegin \DIFadd{, which allows
}\DIFaddend users to automatically \DIFdelbegin \DIFdel{read }\DIFdelend \DIFaddbegin \DIFadd{assess }\DIFaddend their residual plots \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend the trained
computer vision model.

The remainder of this paper is structured as follows:
Section~\DIFaddbegin \DIFadd{\ref{sec-vss-desc} introduces the definition and computation of
visual signal strength. Section~}\DIFaddend \ref{sec-autovi} provides a detailed
...
To train a computer vision model\DIFaddbegin \DIFadd{, }\DIFaddend a measure of the visible pattern in a
plot is needed. We call this the \textbf{visual signal strength} (VSS)\DIFdelbegin \DIFdel{measuring }\DIFdelend \DIFaddbegin \DIFadd{,
which measures }\DIFaddend how prominently a specific set of visual patterns appears
in an image. This can be computed for a training set of data, and plots,
where the generating distributions are specified.

In the context of regression model diagnostics, \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{VSS }\DIFaddend describes the
clarity of visual patterns on a diagnostic plot that may indicate model
violations. \DIFdelbegin \DIFdel{This concept }\DIFdelend \DIFaddbegin \DIFadd{Violations }\DIFaddend can be categorized as weak, moderate, or strong\DIFdelbegin \DIFdel{. However,
in our study, }\DIFdelend \DIFaddbegin \DIFadd{,
but here }\DIFaddend we treat it as a continuous positive real variable.
...
Consequently, the calculation of VSS can \DIFdelbegin \DIFdel{be }\DIFdelend vary across different model
classes or within the same model, depending on the generating function.

VSS \DIFdelbegin \DIFdel{is defined as an estimate of a distance measure that quantifies the disparity }\DIFdelend \DIFaddbegin \DIFadd{estimates the distance }\DIFaddend between the residual distribution of a fitted
classical normal linear regression model and a reference distribution
\DIFdelbegin \DIFdel{, and more details
can be found in \mbox{%DIFAUXCMD
\citet{li2024automated}}\hskip0pt%DIFAUXCMD
. This }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[see][for details]{li2024automated}}\hskip0pt%DIFAUXCMD
. The distance }\DIFaddend measure is based
...
\DIFdelbegin \begin{displaymath} \DIFdel{%DIFDELCMD < \label{eq:kl-1}%%%
D_{KL} = \int_{\mathbb{R}^{n}}\log\frac{p(\boldsymbol{e})}{q(\boldsymbol{e})}p(\boldsymbol{e})d\boldsymbol{e},
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation} \DIFadd{\label{eq:kl-1}
D_{KL} = \int_{\mathbb{R}^{n}}\log\frac{p(\boldsymbol{e})}{q(\boldsymbol{e})}p(\boldsymbol{e})d\boldsymbol{e},
...
This distance measure \DIFdelbegin \DIFdel{requires }\DIFdelend \DIFaddbegin \DIFadd{depends on }\DIFaddend knowledge of the true residual
distribution, which is \DIFdelbegin \DIFdel{typically unknown . Therefore, a }\DIFdelend \DIFaddbegin \DIFadd{unknown in practice. To compute \(D_{KL}\) for
...
\begin{equation*} \DIFadd{\label{eq:kl-2}
...
\DIFadd{The computer vision model approximates this mapping from a set of
...
can be explicitly calculated, enabling supervised training. The }\DIFaddend computer
vision model \DIFdelbegin \DIFdel{was used for estimating the residual distribution \mbox{%DIFAUXCMD
\citep[see][ for
more details]{li2024automated}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{takes a residual plot as input and outputs the
...
\DIFaddbegin \section{\DIFadd{Definition and simulation of null and bootstrapped
residuals}}\label{sec-null-and-boot-desc}

\DIFadd{In the subsequent sections, we will frequently refer to null residuals
and bootstrapped residuals, so it is helpful to first define and explain
how they are generated.
}

\textbf{\DIFadd{Null residuals}} \DIFadd{are used to generate null plots within the
...
\DIFadd{Our computer vision model is trained to assign lower VSS to null plots
...
\textbf{\DIFadd{Bootstrapped residuals}} \DIFadd{are obtained by refitting the model on
...
\DIFaddend \section{R package: autovi}\label{sec-autovi}
...
\textbf{i}nference. \DIFdelbegin %DIFDELCMD <

%DIFDELCMD < %%%
\DIFdel{There are two ways to access the package, directly using R }\DIFdelend \DIFaddbegin \DIFadd{This functionality can be accessed through the R
package }\texttt{\DIFadd{autovi}}\DIFadd{, }\DIFaddend or through a web interface,
\texttt{autovi.web}\DIFdelbegin \DIFdel{. The web interface has the advantage
that it can be used without
installing Python, R and the relevant
packages }\DIFdelend \DIFaddbegin \DIFadd{, which enables users to perform analyses without
installing R, Python, or their associated dependencies }\DIFaddend locally.

\subsection{Motivation\DIFdelbegin \DIFdel{for usage}\DIFdelend }\label{sec-why}
...
\citep{surreal}\DIFdelbegin \DIFdel{where structured residuals are hidden in data, to be
revealed if the correct model is specified. Here a quote based on Tukeyis used as the residual structure }\DIFdelend \DIFaddbegin \DIFadd{, where structure residuals are embedded in the data. In
this case, a quote inspired by Tukey, }\DIFaddend ``visual summaries focus on
unexpected values''\DIFaddbegin \DIFadd{, is used to define the residual structure}\DIFaddend . The
automated plot reading predicts the VSS to be 5.87, with a \(p\)-value
less than 0.05. \DIFdelbegin \DIFdel{This structure is blindingly obvious
visually}\DIFdelend \DIFaddbegin \DIFadd{Visually, the structure is strikingly clear}\DIFaddend , but a RESET
...
%DIFDELCMD < \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_submission1_files/figure-pdf/fig-three-examples-1.pdf}
%DIFDELCMD <
...
input tensors for further analysis. \DIFdelbegin \DIFdel{The }\DIFdelend \texttt{TensorFlow}
\citep{abadi2016tensorflow}\DIFdelbegin \DIFdel{library}\DIFdelend , a key component of modern machine learning,
is used to predict the VSS of these plots \DIFdelbegin \DIFdel{through }\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend a pre-trained
...
saved as PNG files for visual input. \DIFdelbegin \DIFdel{The }\DIFdelend \texttt{cassowaryr}
\citep{mason2022cassowaryr} \DIFdelbegin \DIFdel{library }\DIFdelend computes scagnostics (scatter plot
...
computer vision model. \DIFdelbegin \DIFdel{The }\DIFdelend \texttt{reticulate} \citep{reticulate} \DIFdelbegin \DIFdel{package bridges R and Python, enabling }\DIFdelend \DIFaddbegin \DIFadd{enables
}\DIFaddend seamless communication between \DIFdelbegin \DIFdel{the two languages and supporting the integrated infrastructure}\DIFdelend \DIFaddbegin \DIFadd{R and Python}\DIFaddend .
...
\DIFdelbegin \DIFdel{The code discussed in this paper is based on }\DIFdelend \DIFaddbegin \DIFadd{This paper uses }\DIFaddend \texttt{autovi} version 0.4\DIFdelbegin \DIFdel{.1. }%DIFDELCMD <

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.2. }\DIFaddend The package includes
...
\item
  ...
  bootstrapped plots\DIFdelbegin \DIFdel{, storing }\DIFdelend \DIFaddbegin \DIFadd{. The method stores }\DIFaddend the predictions internally \DIFdelbegin \DIFdel{. A concise report of the check results is then printed}\DIFdelend \DIFaddbegin \DIFadd{and
  prints a concise results report}\DIFaddend .
\end{enumerate}
...
%DIF > <AUTO_VI object>
 Status:
  - Fitted model: lm
%DIF <  - Keras model: UNKNOWN
%DIF >   - Keras model: (None, 32, 32, 3) + (None, 5) -> (None, 1)
     - Output node index: 1
  - Result:
%DIF <     - Observed visual signal strength: 3.162 (p-value = 0.0396)
%DIF >      - Observed visual signal strength: 3.16 (p-value = 0.0396)
...
%DIF <           ║0.8021 1.1109 1.5751 1.6656 1.9199 2.6564 3.3491 ║
%DIF <           ╚═════════════════════════════════════════════════╝
%DIF >            ║0.802 1.111 1.575 1.666 1.919 2.657 3.348 ║
%DIF >            ╚══════════════════════════════════════════╝
     - Bootstrapped visual signal strength: [100 draws]
%DIF <        - Mean: 2.786 (p-value = 0.05941)
%DIF >         - Mean: 2.795 (p-value = 0.05941)
        - Quantiles:
           ╔══════════════════════════════════════════╗
           ║  25%   50%   75%   80%   90%   95%   99% ║
%DIF <           ║2.452 2.925 3.173 3.285 3.463 3.505 3.652 ║
%DIF >            ║2.455 2.941 3.177 3.300 3.474 3.537 3.668 ║
           ╚══════════════════════════════════════════╝
%DIF <     - Likelihood ratio: 0.7275 (boot) / 0.06298 (null) = 11.55
%DIF >      - Likelihood ratio: 0.7333 (boot) / 0.06284 (null) = 11.67
\end{DIFverbatim}
\DIFmodend
...
%DIFDELCMD < \includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{autovi_paper_submission1_files/figure-pdf/fig-plot-pair-1.pdf}
%DIFDELCMD <
...
\includegraphics[width=0.45\linewidth,height=\textheight,keepaspectratio]{autovi_paper_files/figure-pdf/fig-plot-pair-1.pdf}
...
%DIFDELCMD < \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_submission1_files/figure-pdf/fig-summary-plot-1.pdf}
%DIFDELCMD <
...
%DIFDELCMD < \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_submission1_files/figure-pdf/fig-autovi-diag-1.png}
%DIFDELCMD <
...
The \DIFdelbegin \DIFdel{modules }\DIFdelend \DIFaddbegin \DIFadd{package takes regression models and a }\texttt{\DIFadd{Keras}} \DIFadd{model as
primary inputs. Modules }\DIFaddend for VSS prediction and \(p\)-value computation
are \DIFdelbegin \DIFdel{predefined and cannot be overridden, although users can interact with
...
checking their input shapes}\DIFdelend \DIFaddbegin \DIFadd{fixed but accessible via function arguments, using
}\texttt{\DIFadd{TensorFlow}} \DIFadd{for inference and statistical testing. The image
loading module is also fixed, using }\texttt{\DIFadd{PIL}} \DIFadd{to read and resize
images based on the }\texttt{\DIFadd{Keras}} \DIFadd{model's input shape}\DIFaddend . The remaining
seven modules are \DIFdelbegin \DIFdel{designed to
be overridable, enabling users to tailor the infrastructure to their
specific needs. }\DIFdelend \DIFaddbegin \DIFadd{overridable, allowing users to adapt the workflow as
needed. The data extraction module extracts a }\texttt{\DIFadd{data.frame}}
\DIFadd{containing variables used in the regression model. The bootstrapping and
...
}\texttt{\DIFadd{data.frame}}\DIFadd{. The auxiliary computation module calculates
scagnostics such as monotonicity. The plotting module generates a
}\texttt{\DIFadd{ggplot}} \DIFadd{in a standard format, and the plot saving module exports
it at the same resolution as the training images. }\DIFaddend These modules are
\DIFdelbegin \DIFdel{discussed in detail on the software's
website. }\DIFdelend \DIFaddbegin \DIFadd{described in detail in the package documentation.
}\DIFaddend

\DIFaddbegin \subsection{\DIFadd{Extension to Other Model
Classes}}\label{extension-to-other-model-classes}

\DIFadd{The }\texttt{\DIFadd{autovi}} \DIFadd{R package can be extended to accommodate other
classes of models beyond linear regression, such as generalized linear
models (}\texttt{\DIFadd{glm}}\DIFadd{). This is achieved by substituting the relevant
overridable modules, and if needed, supplying a different }\texttt{\DIFadd{Keras}}
\DIFadd{model.
}

\DIFadd{We provide an example of defining a new checker class tailored for
Poisson regression using the }\texttt{\DIFadd{glm}} \DIFadd{framework:
}
...
\tightlist
\item
  \DIFadd{Define a new class using }\texttt{\DIFadd{new\_class()}} \DIFadd{with }\texttt{\DIFadd{AUTO\_VI}}
  \DIFadd{as the parent class.
}\item
  \DIFadd{Override the necessary methods using }\texttt{\DIFadd{register\_method()}}\DIFadd{. In
  ...
  \DIFadd{Create an alias for the }\texttt{\DIFadd{instantiate()}} \DIFadd{method of the new
  class.
}\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\DIFadd{\NormalTok{AUTO\_POIS\_VI }\OtherTok{\textless{}{-}} \FunctionTok{new\_class}\NormalTok{(AUTO\_VI, }\AttributeTok{class\_name =} \StringTok{"AUTO\_POIS\_VI"}\NormalTok{)}
\FunctionTok{register\_method}\NormalTok{(}
\NormalTok{  AUTO\_POIS\_VI,}
  \AttributeTok{get\_fitted\_and\_resid =} \ControlFlowTok{function}\NormalTok{(}\AttributeTok{fitted\_model =}\NormalTok{ self}\SpecialCharTok{$}\NormalTok{fitted\_model) \{}
    \FunctionTok{tibble}\NormalTok{(}\AttributeTok{.fitted =} \FunctionTok{fitted}\NormalTok{(fitted\_model),}
           \AttributeTok{.resid =} \FunctionTok{resid}\NormalTok{(fitted\_model, }\AttributeTok{type =} \StringTok{"pearson"}\NormalTok{))}
\NormalTok{  \},}
  \AttributeTok{null\_method =} \ControlFlowTok{function}\NormalTok{(}\AttributeTok{fitted\_model =}\NormalTok{ self}\SpecialCharTok{$}\NormalTok{fitted\_model) \{}
\NormalTok{    dat }\OtherTok{\textless{}{-}} \FunctionTok{model.frame}\NormalTok{(fitted\_model)}
\NormalTok{    dat[[}\DecValTok{1}\NormalTok{]] }\OtherTok{\textless{}{-}} \FunctionTok{rpois}\NormalTok{(}\FunctionTok{nrow}\NormalTok{(dat), }\AttributeTok{lambda =} \FunctionTok{fitted}\NormalTok{(fitted\_model))}
\NormalTok{    new\_mod }\OtherTok{\textless{}{-}} \FunctionTok{update}\NormalTok{(fitted\_model, }\AttributeTok{data =}\NormalTok{ dat)}
    \FunctionTok{return}\NormalTok{(self}\SpecialCharTok{$}\FunctionTok{get\_fitted\_and\_resid}\NormalTok{(new\_mod))}
\NormalTok{  \}}
\NormalTok{)}
\NormalTok{auto\_pois\_vi }\OtherTok{\textless{}{-}}\NormalTok{ AUTO\_POIS\_VI}\SpecialCharTok{$}\NormalTok{instantiate}
}\end{Highlighting}
\end{Shaded}

\DIFadd{The resulting checker class can be employed analogously to the linear
...
manifests as a pronounced U-shaped pattern in the lineup display (see
Figure~\ref{fig-pois-lineup}), which is also successfully identified by
the computer vision model, yielding a p-value substantially below the
conventional threshold of 0.05.
}

\begin{Shaded}
\begin{Highlighting}[]
\DIFadd{\NormalTok{x }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(}\DecValTok{300}\NormalTok{, }\AttributeTok{sd =} \FloatTok{0.5}\NormalTok{)}
\NormalTok{y }\OtherTok{\textless{}{-}} \FunctionTok{rpois}\NormalTok{(}\DecValTok{300}\NormalTok{, }\AttributeTok{lambda =} \FunctionTok{exp}\NormalTok{(}\DecValTok{1} \SpecialCharTok{+}\NormalTok{ x }\SpecialCharTok{+}\NormalTok{ x}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{))}
\NormalTok{pois\_checker }\OtherTok{\textless{}{-}} \FunctionTok{auto\_pois\_vi}\NormalTok{(}
  \FunctionTok{glm}\NormalTok{(y }\SpecialCharTok{\textasciitilde{}}\NormalTok{ x, }\AttributeTok{family =} \StringTok{"poisson"}\NormalTok{),}
  \AttributeTok{keras\_model =} \FunctionTok{get\_keras\_model}\NormalTok{(}\StringTok{"vss\_phn\_32"}\NormalTok{)}
\NormalTok{)}
\NormalTok{pois\_checker}\SpecialCharTok{$}\FunctionTok{plot\_lineup}\NormalTok{()}
}\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{autovi_paper_files/figure-pdf/fig-pois-lineup-1.pdf}

}

\caption{\label{fig-pois-lineup}\DIFaddFL{A lineup of residual plots from Poisson
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFadd{\NormalTok{pois\_checker}\SpecialCharTok{$}\FunctionTok{check}\NormalTok{()}
}\end{Highlighting}
\end{Shaded}

\DIFmodbegin
\begin{DIFverbatim}[alsolanguage=DIFcode]
%DIF > <AUTO_POIS_VI object>
%DIF >  Status:
%DIF >   - Fitted model: glm, lm
%DIF >   - Keras model: (None, 32, 32, 3) + (None, 5) -> (None, 1)
%DIF >      - Output node index: 1
%DIF >   - Result:
%DIF >      - Observed visual signal strength: 4.875 (p-value = 0.009901)
%DIF >      - Null visual signal strength: [100 draws]
%DIF >         - Mean: 1.331
%DIF >         - Quantiles:
%DIF >            ╔══════════════════════════════════════════╗
%DIF >            ║  25%   50%   75%   80%   90%   95%   99% ║
%DIF >            ║1.035 1.233 1.488 1.644 1.941 2.276 2.639 ║
%DIF >            ╚══════════════════════════════════════════╝
%DIF >      - Bootstrapped visual signal strength: [100 draws]
%DIF >         - Mean: 5.51 (p-value = 0.009901)
%DIF >         - Quantiles:
%DIF >            ╔══════════════════════════════════════════╗
%DIF >            ║  25%   50%   75%   80%   90%   95%   99% ║
%DIF >            ║5.330 5.505 5.698 5.735 5.830 5.903 6.013 ║
%DIF >            ╚══════════════════════════════════════════╝
%DIF >      - Likelihood ratio: 0.05096 (boot) / 0 (null) = Extremely large
\end{DIFverbatim}
\DIFmodend

\DIFadd{It is important to note, however, that the pre-trained computer vision
model included in }\texttt{\DIFadd{autovi}}\DIFadd{, such as }\texttt{\DIFadd{vss\_phn\_32}} \DIFadd{(see
}\texttt{\DIFadd{list\_keras\_models()}} \DIFadd{for the full list of available models),
...
cases, users are encouraged to train and apply their own }\texttt{\DIFadd{Keras}}
\DIFadd{models. Detailed guidance on model training and discussion on extending
...
\DIFaddend \section{Web interface: autovi.web}\label{sec-autovi-web}
...
user's browser. However, \DIFdelbegin \DIFdel{this approach was not feasible at the time of
writing this paper. The reason is that one of the R packages }\texttt{\DIFdel{autovi}} %DIFAUXCMD
\DIFdel{depends on the R package }\DIFdelend \DIFaddbegin \texttt{\DIFadd{webr}} \DIFadd{does not support packages which
use compiled fortran code, which is required by }\DIFaddend \texttt{splancs}
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{splancs}}\hskip0pt%DIFAUXCMD
, which
uses compiled Fortran code. A }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{splancs}}\hskip0pt%DIFAUXCMD
, a dependency of }\texttt{\DIFadd{autovi}}\DIFadd{. In the future, it is
possible that a }\DIFaddend working Emscripten \citep{zakai2011emscripten} version
of this package \DIFdelbegin \DIFdel{, which would be
required for }\DIFdelend \DIFaddbegin \DIFadd{may allow full }\DIFaddend \texttt{webr} \DIFdelbegin \DIFdel{, was not available}\DIFdelend \DIFaddbegin \DIFadd{support}\DIFaddend .
...
Amazon Web Services (AWS). However, \DIFdelbegin \DIFdel{correctly setting up the server
and
ensuring a secure deployment requires significant }\DIFdelend \DIFaddbegin \DIFadd{deploying and maintaining the server
securely requires some }\DIFaddend expertise. Ultimately, the most practical
...
Due to \DIFdelbegin \DIFdel{the resource allocation policy of }\DIFdelend shinyapps.io\DIFaddbegin \DIFadd{'s resource policy, inactive servers enter sleep
mode}\DIFaddend , \DIFdelbegin \DIFdel{the server enters
a sleep
modeduring periods of inactivity, resulting in the clearing of
}\DIFdelend \DIFaddbegin \DIFadd{clearing }\DIFaddend the local Python \DIFdelbegin \DIFdel{virtual environment. Consequently, when the application
``wakes up'' }\DIFdelend \DIFaddbegin \DIFadd{environment. When reactivated }\DIFaddend for a new
\DIFdelbegin \DIFdel{user session, these libraries need to }\DIFdelend \DIFaddbegin \DIFadd{session, libraries must }\DIFaddend be reinstalled. While this ensures a clean
environment for each session, it may lead to slightly longer loading
times for the first user after a period of inactivity.

In contrast to \texttt{autovi}, \texttt{autovi.web} \DIFdelbegin \DIFdel{does not use the
native Python version of }\texttt{\DIFdel{TensorFlow}}%DIFAUXCMD
\DIFdel{. Instead, it }\DIFdelend leverages
...
%DIFDELCMD < \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_submission1_files/figure-pdf/fig-autovi-web-setup-1.png}
%DIFDELCMD <
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_files/figure-pdf/fig-autovi-web-setup-1.png}
...
assessment. \DIFaddbegin \DIFadd{The attention map is the gradient of the model output with
...
%DIFDELCMD < \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_submission1_files/figure-pdf/fig-autovi-web-lineup-1.png}
%DIFDELCMD <
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_files/figure-pdf/fig-autovi-web-lineup-1.png}
...
%DIFDELCMD < \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_submission1_files/figure-pdf/fig-autovi-web-distributions-1.png}
%DIFDELCMD <
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{autovi_paper_files/figure-pdf/fig-autovi-web-distributions-1.png}
