...
\hypersetup{
  pdftitle={Automated residual plot assessment with the  package  and the  app },
...
\textsf{autovi} and the \textsf{Shiny} app \textsf{autovi.web}}
...
\hspace*{1ex} Email: \texttt{patrick.li@anu.edu.au}    
}
...
evaluation of residual plots. An accompanying \textsf{Shiny} app is
provided for ease of use. Given a sample of residuals, the model
...
autovi}{ package: autovi}}\label{sec-autovi}
...
\textsf{R} package \texttt{autovi}, or through a web interface,
...
\textsf{Python}, \texttt{pillow} \citep{clark2015pillow} handles image
...
\texttt{TensorFlow} \citep{abadi2016tensorflow}, a key component of
modern machine learning, is used to predict the VSS of these plots using
a pre-trained convolutional neural network.

In the \textsf{R} environment, \texttt{autovi} utilizes several
libraries. \texttt{ggplot2} \citep{ggplot2} generates the initial
residual plots, saved as PNG files for visual input. \texttt{cassowaryr}
...
by the \texttt{reticulate} package. If the necessary \textsf{Python}
...
\texttt{reticulate::use\_python()} function before using the
\texttt{autovi} package.

We recommend using the \textsf{Shiny} app \texttt{autovi.web} if users
...
\includegraphics[width=0.45\textwidth,height=\textheight]{autovi_paper_files/figure-pdf/fig-plot-pair-1.pdf}
...
of the extsf\{R\} package \texttt{autovi}. The modules in green are
...
The \texttt{autovi} \textsf{R} package can be extended to accommodate
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{AUTO\_POIS\_VI }\OtherTok{\textless{}{-}} \FunctionTok{new\_class}\NormalTok{(AUTO\_VI, }\AttributeTok{class\_name =} \StringTok{"AUTO\_POIS\_VI"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(}\DecValTok{300}\NormalTok{, }\AttributeTok{sd =} \FloatTok{0.5}\NormalTok{)}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{autovi_paper_files/figure-pdf/fig-pois-lineup-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pois\_checker}\SpecialCharTok{$}\FunctionTok{check}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
The \texttt{autovi.web} \textsf{Shiny} application extends the
functionality of \texttt{autovi} by offering a user-friendly web
...
implementation based on \texttt{autovi.web} version 0.1.0.

\subsection{Implementation}\label{implementation}

The interface \texttt{autovi.web} is built using the \texttt{shiny}
\citep{shiny} and \texttt{shinydashboard} \citep{shinydashboard}
...
\texttt{htmltools} \citep{htmltools} and \texttt{shinycssloaders}
...
using the \texttt{rpy2} \textsf{Python} library \citep{rpy2}, but this
...
\texttt{shiny} and \texttt{shinydashboard} frameworks, which are
...
libraries, including \texttt{pillow} and \texttt{numpy}, are
pre-installed on the server. These libraries are integrated into the
\textsf{Shiny} application using the \texttt{reticulate} package, which
provides an interface between \textsf{R} and \textsf{Python}.

Due to shinyapps.io's resource policy, inactive servers enter sleep
mode, clearing the local \textsf{Python} environment. When reactivated
for a new session, libraries must be reinstalled. While this ensures a
clean environment for each session, it may lead to slightly longer
loading times for the first user after a period of inactivity.
...
initializes, \texttt{TensorFlow.js} rebuilds the computer vision model
using these pre-stored weights.

To allow communication between \texttt{TensorFlow.js} and other
components of the \textsf{Shiny} application, the \texttt{shinyjs}
\textsf{R} package \citep{shinyjs} is used. This package allows calling
custom JavaScript code within the \textsf{Shiny} framework. The
specialized JavaScript code for initializing \texttt{TensorFlow.js} and
calling \texttt{TensorFlow.js} for VSS prediction is deployed alongside
the \textsf{Shiny} application as additional resources.
...
package \texttt{autovi} and its accompanying web interface,
...
The \texttt{autovi} \textsf{R} package, introduced in this paper,
...
\texttt{autovi.web}, expands access to these advanced diagnostic tools,
